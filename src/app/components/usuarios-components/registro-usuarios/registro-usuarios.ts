...
export class RegistroUsuarios {
  ...
  constructor(private fb: FormBuilder, private router: Router, private usuariosService: UsuariosService) {
    this.registroForm = this.fb.group({
      ...
      documento: ['', [Validators.required, Validators.pattern(/^[A-Za-z0-9]{6,20}$/)]],
  ...
  camposSinLlenar = () => {
    return !this.enviado && Object.values(this.registroForm.controls).some(control => control.value?.trim?.() !== '');
  }
}
